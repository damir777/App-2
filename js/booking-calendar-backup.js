var HotelDatepicker = (function () {
    ...
    var HotelDatepicker = function HotelDatepicker(input, options) {
        ...
        this.seasonMonths = opts.seasonMonths || false;
        this.enableDocumentClick = opts.enableDocumentClick || false;
        this.selectForward = opts.selectForward || false;
        this.disabledDates = opts.disabledDates || [];
        this.invalidDates = opts.invalidDates || [];
        this.disabledMonths = opts.disabledMonths || [];
        ...
        this.showClearTopbar = opts.showClearTopbar === undefined ? false : opts.showClearTopbar;
    ...
    HotelDatepicker.prototype.createDatepickerDomString = function createDatepickerDomString () {
        ...
        if (this.showClearTopbar)
        {
            html += '<div class="datepicker__topbar"><button type="button" class="btn btn-primary clear-booking-selection">' +
                this.lang('clear') + '</button></div>';
        }
    ...
    HotelDatepicker.prototype.createMonthDomString = function createMonthDomString (_date) {
        ...
        if ((m + 1) >= this$1.seasonMonths[0] && (m + 1) <= this$1.seasonMonths[1]) {
            this$1.disabledDaysOfWeek = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Sunday"];
        } else {
            this$1.disabledDaysOfWeek = [];
        }
        ...
        for (var i$1 = 0; i$1 < 40; i$1++) {
            ...
            if (this.disabledMonths.indexOf(m + 1) !== -1) {
                valid = false;
            }
        ...
        for (var week = 0; week < 6; week++) {
            ...
            for (var i$2 = 0; i$2 < 7; i$2++) {
                ...
                if (_day$2.valid) {
                    ...
                    if (this$1.invalidDates.length > 0) {
                        if (this$1.invalidDates.indexOf(dateString) > -1) {
                            _day$2.valid = false;
                            isDisabled = true;
    ...
    HotelDatepicker.prototype.datepickerHover = function datepickerHover (evt) {
        ...
        if (this.wholeWeek && evt.target.tagName && evt.target.tagName.toLowerCase() === 'tr') {
            evt.target.find("td").forEach(function (el) {
                console.log("EL", el);
                this.dayHovering(el);
            })

        }else if (evt.target.tagName && evt.target.tagName.toLowerCase() === 'td') {
    ...
    HotelDatepicker.prototype.showSelectedDays = function showSelectedDays () {
        ...
        var weeks = this.datepicker.getElementsByTagName('tr');
    ...
    HotelDatepicker.prototype.dayClicked = function dayClicked (day) {
        ...
        console.log("Native click disabledDaysOfWeek", this.disabledDaysOfWeek);
    ...
    HotelDatepicker.prototype.isValidDate = function isValidDate (time) {
        ...
        if (this.start && !this.end) {
            ...
            if (this.invalidDates.length > 0) {
                var limit2 = this.getClosestDates(new Date(parseInt(this.start, 10)));

                if (limit2[0] && this.compareDay(time, limit2[0]) <= 0) {
                    return false;
                }

                if (limit2[1] && this.compareDay(time, limit2[1]) >= 0) {
                    return false;
                }
            }
        }
    ...
    HotelDatepicker.prototype.parseInvalidDates = function parseInvalidDates () {
        var this$1 = this;

        // Sort invalid dates and store it in property
        var _tmp = [];
        this.setFechaI18n();
        for (var i = 0; i < this.invalidDates.length; i++) {
            _tmp[i] = fecha.parse(this$1.invalidDates[i], 'YYYY-MM-DD');
        }
